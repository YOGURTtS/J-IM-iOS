...
{
    NSString *path = [NSString stringWithFormat:@"%@/User/%@/message/DB/", [NSFileManager documentsPath], @"1234"];
...
{
    NSString *path = [NSString stringWithFormat:@"%@/User/%@/Frame/DB/", [NSFileManager documentsPath], @"1234"];
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        NSError *error;
        [[NSFileManager defaultManager] createDirectoryAtPath:path withIntermediateDirectories:YES attributes:nil error:&error];
        if (error) {
